...
/** Live-test the Sheng Siong module. Usage: npm run ss -- tofu milk banana */
...
for (const term of terms) {
	console.log(`\n===== Sheng Siong: "${term}" =====`);
	try {
		const products = await shengsiong.search(term);
		...
		for (const p of products.slice(0, 8)) {
			...
			console.log(`  • ${p.name} [${p.brand ?? "-"}]  $${p.priceSgd}  ${p.packWeightG ?? "?"}g  ${per}${sale}`);
		}
	} catch (e: any) {

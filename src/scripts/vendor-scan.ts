...
import { describeSlotDecision, recordDiscount, recordVendorPrice } from "../core/ingredient-write.js";
import {
	ROUTES,
	notionClient,
	pickCandidate,
...
import { discountBeats, formatDiscount } from "../core/discount.js";
...
async function main(): Promise<void> {
	...
	for (const row of rows) {
		...
		for (const [i, { route, slot }] of row.routes.entries()) {
				...
				`    ${suggestion ? "≋" : "✓"} ${money(p.priceSgd)} / ${size}${unitWord(row)}` +
					...
					(p.promoPriceSgd != null
						? `\n      🏷️ on promo at ${money(p.promoPriceSgd)} — recording the ${money(p.priceSgd)} shelf price;` +
							` the offer goes to the discount columns`
						: "") +
			...
			// ⚠️ **A supplement is only recorded without asking when the brand matches**
			// (user, 2026-09-09). Not the `Brand Specific` tag, which makes a wrong brand a
			// hard miss the scan never offers — this makes it a question. Measured the same
			// day: `Omega 3 [california gold]` was filled silently with `Country Life,
			// Natural Omega-3` at 0.750, because every word of the row name was in that
			// title and the brand was the only thing wrong with it.
			const unconfirmed = brandUnconfirmed(row.target, p);
			if (unconfirmed) reasons.unshift(unconfirmed);
			...
			/**
			 * **Today's offer into the three discount columns — beside the shelf price, never
			 * on top of it** (user, 2026-09-12).
			 *
			 * `atShelfPrice` has already replaced `p.priceSgd` with the pre-promo figure by the
			 * time anything here runs, which is what keeps the ratchet honest; `promoPriceSgd`
			 * is the only surviving record of what the shop is charging today, and until now
			 * nothing priced off it. See `DISCOUNT_PROPS`.
			 *
			 * ⚠️ **Tied to the pick that was just RECORDED, and to no other.** The discount
			 * columns name one shop and one pack; filling them from a candidate the price book
			 * refused would advertise an offer on something this row does not price.
			 *
			 * ⚠️ A row scanned at this shop and found NOT on offer clears the cells — but only
			 * where this shop is the one named in them. A promo is the one figure here with an
			 * expiry, and a column nobody empties reads as a live offer forever. What it does
			 * NOT clear is a row where the shop had no match at all this run: no match is not
			 * evidence the offer ended.
			 */
			const syncDiscount = async (): Promise<void> => {
				const promo = p.promoPriceSgd;
				if (promo != null) {
					const text = formatDiscount({
						vendor: route.option,
						unitType: row.unitType,
						promoSgd: promo,
						size,
					...
					if (
						!discountBeats({
							recordedLocation: row.discount.location,
							recordedRate: row.discount.rate,
							vendor: route.option,
							rate: text.rateValue,
						})
					) {
						console.log(
							`      🏷️ offer not recorded — ${row.discount.location} already holds a cheaper one` +
								(row.discount.rate != null ? ` at $${row.discount.rate.toFixed(2)}` : ""),
						);
						return;
					}
					if (!doWrite) {
						console.log(`      🏷️ would record the offer — ${text.price}${text.rate ? `  =  ${text.rate}` : ""}`);
						return;
					}
					const res = await recordDiscount(client, row.pageId, {
						kind: "offer",
						capture: {
							vendor: route.option,
							unitType: row.unitType,
					...
					if (res.action === "wrote") {
						discountsWritten++;
						// Kept in step in memory, so a second shop on offer in this same run is
						// compared against what Notion now holds rather than what it held at read.
						row.discount = { price: text.price, location: text.location, rate: text.rateValue };
						console.log(`      🏷️ discount recorded — ${text.price}${text.rate ? `  =  ${text.rate}` : ""}`);
					} else {
						console.log(`      🏷️ discount not recorded — ${res.reason ?? res.skipped.join("; ")}`);
					}
					return;
				}

				// Not on offer. Clear only what this shop put there.
				if (!row.discount.price && !row.discount.location && row.discount.rate == null) return;
				if (row.discount.location && row.discount.location.toLowerCase() !== route.option.toLowerCase()) {
					return;
				}
				if (!doWrite) {
					console.log(`      🏷️ would clear a discount that has ended (${row.discount.price || "—"})`);
					return;
				}
				const res = await recordDiscount(client, row.pageId, { kind: "clear", vendor: route.option });
				if (res.action === "cleared") {
					discountsCleared++;
					row.discount = { price: "", location: "", rate: null };
					console.log(`      🏷️ discount cleared — the offer has ended`);
				}
			};
			...
			try {
				...
				if (landed) {
					...
					// never happened.
					notePriceMove();
					// ⚠️ AFTER the slot write and only when it landed: the two describe the same
					// pack at the same shop, and a discount beside a price that was refused is
					// an offer on something this row does not sell.
					await syncDiscount();
	...
	console.log(
		`\n${"─".repeat(78)}\n` +
			...
			(discountsWritten ? `, ${discountsWritten} discount(s) recorded` : "") +
			(discountsCleared ? `, ${discountsCleared} ended discount(s) cleared` : "") +

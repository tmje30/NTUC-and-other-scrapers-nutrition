...
chrome.storage.local.get("notionToken").then(({ notionToken }) => { $("token").value = notionToken || ""; });
...
$("save").addEventListener("click", async () => {
  ...
  chrome.runtime.sendMessage({ type: "token-test" }, (res) => {
    ...
    const r = res.result;
    if (r.ok) return setMsg("✓ Token works and the Ingredients DB has every property this writes.", "ok");
    setMsg(`Problems:\n• ${r.problems.join("\n• ")}`, "error");
  });
});
